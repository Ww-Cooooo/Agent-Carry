...
// mock 数据只允许存在于不随公开 main／安装包分发的私密维护夹具中，
// 绝不能伪装成真实助手状态。
...
const GLOBAL_ACTIONS: GlobalActionDef[] = [
  ...
  },
  {
    ...
    request: buildGlobalRequest({
      ...
      routeId: "template-upgrade",
      target: "core/guides/upgrade-guide.md",
      goal: "我现在要检查并可能升级当前 Agent Carry 实例的模板核心。",
      scope: [
        "先只生成替换、迁移、保留、删除、冲突和扩展兼容预览。",
        "升级前创建临时回滚副本；在我明确选择迁移升级后再执行。",
      ],
      forbidden: ["不得静默覆盖实例资产", "不得强制升级"],
      confirmation: "在我明确选择迁移升级后再执行",
      resultFields: ["版本变化", "替换/迁移/保留/冲突清单", "回退状态", "验收结果"],
    }),
  },
  {
