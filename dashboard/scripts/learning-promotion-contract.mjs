...
import { findPotentialFormalDuplicates, loadTrustedDomainEnvelope, parseMarkdownFrontmatterHead, prepareNewFormalTarget, stableAssetId, trustedMaintenanceStateDigest, validateProposedFormalAsset, verifyNewFormalTarget } from "./asset-route-contract.mjs";
...
const consumedConfirmationMessageRefs = new Set();
...
export function preparePromotionReview(repository, { candidateId, formalTarget, formalPreview, mode = "explicit-user", classificationReason = "" } = {}) {
  ...
  if (candidate.candidateRelation !== "new") return fail("non-new-relation-requires-targeted-review");
  let parsed;
  ...
  if (duplicateCheck.matches.length > 0) return fail("possible-formal-duplicate-requires-targeted-review");
  ...
  const review = Object.freeze({
    ...
    confirmationReceiptContract: "current-user-intent-bound-to-current-preview",
  });
  ...
  trustedReviews.set(review, Object.freeze({ repository: repositoryReal, context, envelope, maintenanceStateDigest, candidate, asset, formalTarget, targetProof,
    normalizedPreview, previewDigest, projectedRouteDigest: projectedRoute.digest, promotionChangeGateIds, retainedFutureActionGateIds, mode }));
  return review;
}
...
export function confirmPromotionReview(repository, review, receipt) {
  ...
  const valid = receipt && typeof receipt === "object" && !Array.isArray(receipt)
    ...
    && (receipt.confirmed_change_gate_ids === undefined
      || (Array.isArray(receipt.confirmed_change_gate_ids)
        && trust.promotionChangeGateIds.every((id) => receipt.confirmed_change_gate_ids.includes(id))))
    && !consumedConfirmationMessageRefs.has(messageKey);
  ...
  consumedConfirmationMessageRefs.add(messageKey);
  ...
  trustedConfirmations.set(confirmation, Object.freeze({ ...trust, currentContext: current.context }));
  return confirmation;
}
...
export function buildPromotionTransactionPlan(repository, confirmation) {
  ...
  if (!trust || !candidateTrust || consumedConfirmations.has(confirmation)
  ...
  const planCore = {
    ...
    requiredCoreSet: Object.freeze(["formal-asset", "instance-domain-map", "archived-source-candidate"]),
    bestEffortProjectionSet: Object.freeze(["evolution-candidate-index", "matching-learning-signal-if-present", "dashboard-public-snapshot", "dashboard-dist-snapshot"]),
    requiredAudits: Object.freeze(["formal-source-closure", "candidate-source-closure", "core-write-rollback", "projection-failure-is-local", "second-run-idempotence"]),
    preCommitRequirements: Object.freeze(["reverify-manifest-map-candidate-target-preimage", "stage-semantic-core-set", "rollback-core-only-on-core-failure"]),
  };
  ...
  trustedPlans.set(plan, Object.freeze({ repository: realpathSync(repository), candidateId: trust.candidate.id, formalId: trust.asset.id,
    formalPreview: trust.normalizedPreview, formalPreviewDigest: trust.previewDigest }));
  return plan;
}

export function projectPromotionState(state, plan) {
  const trust = trustedPlans.get(plan);
  if (!trust || !state || typeof state !== "object") throw new Error("untrusted promotion plan");
...
// Cross-process promotion consumes the exact reviewed handoff persisted by the
