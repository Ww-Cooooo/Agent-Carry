...
- **当前阶段**：`1.4.0` 正式发布契约；在 `1.3.1` 基线上增加实例持续进化兼容协定、小型组件登记、旧实例一次性纳管和接口闭包；实例替换仍必须同时核对固定标签、Release、清单、解压树和用户对该次升级的明确选择，且不授权未来仓库／发布动作
- **最近更新**：2026-08-23
...
| 实例持续进化与母版升级兼容 | 1.4.0 候选 | 所有 Agent Carry 发起的正式持久变化命中同一轻量兼容入口，但不增加用户确认；原生资产与专业扩展复用现有所有者，独立组件使用小注册表。旧实例在隔离候选中一次性纳管；兼容则保留，可选不兼容则停用并保留，必需不兼容则停止切换；普通启动不读取，待固定正式发布与真实实例验收 |
