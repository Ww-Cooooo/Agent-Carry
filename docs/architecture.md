...
- **当前阶段**：AI Carry `2.0.8`；继续沿用四项最高原则与 2.0.5 的精简架构，在现有 Skill 工坊内加入跨 Agent 创作核心：复用对话和正式资产、只追问真实缺口、生成开放格式、兼容旧包，并把宿主原生 Creator 保持为可选复核；不增加后台服务、Schema、依赖、市场或批量评测。1.4.8、未公开 1.4.9、2.0.0、2.0.1、2.0.2、2.0.3、2.0.4、2.0.5、2.0.6 与 2.0.7 都是直接来源
- **最近更新**：2026-09-04
...
| 首次实例化身份与发布生命线 | 2.0.4 基线，2.0.8 继续保留 | 正式创建由随包执行器接收一份已确认的紧凑语义请求，只生成并原子提交 manifest、正式档案和领域地图；模型不逐文件手写。空索引、注册表、胶囊和快照按能力需要延迟初始化或重建。代表性首次创建验证核心回滚、派生失败局部降级和二次幂等；已有实例升级另用复杂合成样例核对用户、私密、本机和扩展内容保留。普通用户不运行维护测试 |
...
| Skill 工坊与共享 Skill 接入 | 1.4.4 基线，2.0.8 内置创作核心 | 推荐只读且不自动转换；用户选中的正式方法才复制为本地隔离草稿。内置核心从当前对话和资产提取已有意图，只追问会改变成品的缺口，自动脱敏、通用化并写成开放 Agent Skills 结构；宿主原生 Creator 只作可选复核，缺失或失败不阻断。普通 Skill 只核对少量真实触发与一个不应触发场景，复杂且可客观验证时才增加一条代表任务。外来 Skill 仍只读检查且不执行脚本；新格式在标准 metadata 保存共享身份和版本，旧格式继续读取，冲突只隔离当前包。导出索引按需创建，不进入普通启动；已有 Skill、分享、上传、安装和升级边界保持不变 |
