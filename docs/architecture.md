...
| 首次实例化身份与发布生命线 | 1.4.1 基线，1.4.8 确定性入口 | 全部模板身份文件、严格启动胶囊和双快照进入同一可恢复事务；结果验证索引保持零记录、候选索引只写一次真实创建时间、组件登记表为零组件 current/revision=1。每个候选用同一空白真源完成通用与剪辑领域两条轻量首次创建链，并单独验证空模板和复杂合成实例的补丁升级；正式创建由随包的确定性事务执行器接收一份已确认的紧凑语义请求，统一生成、严格回读并原子提交全部身份闭包；模型不再手写正式文件。局部失败恢复模板并只暂停当前创建，普通对话和无关能力继续可用；普通用户不运行维护测试 |
