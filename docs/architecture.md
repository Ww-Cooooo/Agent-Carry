...
- **当前阶段**：`1.1.3`；在 `1.1.2` 基线上增加按需正式修改质量门、旧实例档案安全迁移、任务族状态规范和正式资产可达性闭包
- **最近更新**：2026-08-18
...
- 完整换机迁移使用宿主无关的固定套件契约：普通主体与本地隐私保持两个包，逐文件清单和 SHA-256 在写入目标目录前核对，秘密凭据在目标环境重新配置。
...
首次使用的四个高风险退出点采用事件局部检查，而不是增加普通启动上下文：安装入口验证后、模板态选中首项任务且准备索取实际数据时、实例化正式写入前后、第一项真实任务收到“结果正确／以后继续使用／保存方法”等反馈时，当前 Agent 重新读取 `core/guides/first-use-execution-gates.md` 的对应小节。第二个检查点阻止“先做一次任务试试看”绕过正式方向与实例化：用户明确选择通用或专业方向，并明确确认当前模型处于 Level 3（不足时手动切换）后，才进入实例结构设计、完整预览和写入。实例化完成门同时要求三张治理卡生成首轮排期、当前宿主建立一份最小档案、信号与时间索引一致、看板只投影真实正式资产。第一项真实任务通过验证且用户明确要求以后复用时，方法才能形成 `practiced` 资产；只确认结果正确不自动授权新资产。
...
“来源能证明什么”“为什么允许使用”“错误后果多大”和“真实任务验证到哪里”是四个不同维度，不能把所有其他 Agent 输出一律称为不可信候选，也不能把用户批准等同于已经可靠。Asset Schema 1.2 分别使用来源、`approval_state/activation_basis`、`risk_tier` 和 `maturity` 表达。这里的 `Asset Schema 1.2` 是内部资产结构基线，不是 Agent Carry 产品版本；产品版本与各组件基线分别演进。

| 内容来源 | 默认处理 | 资产状态 |
| --- | --- | --- |
| Agent Carry 提供后由已接入宿主原样返回的内容 | 识别来源并去重 | 保持原状态，不重复导入 |
| 用户在已接入会话中明确确认或要求记住的决定 | 用户权威内容 | 可按规则正式沉淀 |
| 模型和宿主根据任务提出的新经验 | 正常学习成果，检查证据和范围 | 通常先成为候选 |
...
6. 用户确认需要其判断的内容；低风险内容按分级规则处理。

Agent Carry 的“自我进化”不是修改基础模型参数，而是让可携带资产随着真实工作变得更完整、更准确、更容易被未来模型复用。

### 10.2 分级沉淀

**当前基线（Asset Schema 1.2）**：

- 用户明确说“记住这个”或“以后都这样”：视为明确授权，不重复询问同一决定。
- 模型根据一次任务推断出的偏好或经验：先成为候选。
- 多次真实任务反复验证的低风险规律：可以渐进提升成熟度，并用自然语言通知用户，保留撤销方式。
- 身份、隐私、安全规则、重要偏好和高影响 SOP：必须由用户确认。
- 临时路径、一次性参数和过程噪声：不进入长期资产。
- 来自外部资料的结论：保留来源并核实，不能因模型总结过就直接晋升。

`status`、授权依据、风险和成熟度彼此独立。低风险内容满足独立真实证据、无冲突、通知和可撤销条件后，先进入 `provisional`；试用后再次成功复用才可进入 `active`。授权来自用户在实例化时选择的政策，不来自沉默。用户可以把实例改为 `manual-only`。
...
新学习先整理为仅供匹配使用的检索特征，Asset Schema 1.2 已为候选提供对应字段，至少包含：
...
“同类候选反复出现”默认表示：同一学习主题在至少两个相互独立的任务事件中，再次匹配到相同主题、核心主张、适用范围和条件。消息改写、工具重试、会话恢复和宿主原样回传不增加独立计数。次数只触发检查，不自行产生授权；只有生命周期的低风险、真实证据、无冲突、通知和可撤销条件同时成立时才进入试用。正式冲突、用户明确替代和高风险异常可以立即触发。
...
**当前基线（Asset Schema 1.2）**包含以下正式角色：
...
**当前基线补充**：不能仅按“是不是其他 AI 输出”判断信任。已接入 Agent Carry 的宿主观察、用户明确决定、宿主协作记忆、模型推断、外部资料和未知来源已经通过接入协议与安全边界分别标记。宿主转述的网页、工具结果和未经验证推断仍保持原来源，不能因为宿主已接入而洗白；任何来源都不能伪装成用户当前直接授权，也不能单独触发低风险政策晋升。
...
3. 用户实例默认保留在本地。用户可以自愿把经过检查的非隐私内容备份到自己账号下的 GitHub 私密仓库，但 Agent Carry 不强制要求 Git、GitHub 或某种发布流程。
4. 本地隐私迁移包与 GitHub 私密备份是两条独立通道：前者只用于用户控制的设备间迁移，永不进入 Git；后者默认不包含隐私正文或秘密凭据，远程创建或推送前还要确认账号、仓库名、`private` 可见性、分支和候选摘要。
...
正式可迁移资产与本地私密资产分层：Git 安全资产只保存低敏摘要、路由和 `private_refs`；原始隐私正文位于 `.assistant-private/assets/`，只在任务命中后按需读取。用户要求 GitHub 私密备份时先生成全新洁净候选副本，同时检查路径和普通文档／配置／源码内容，不能只依赖 `.gitignore`；本地检查通过后展示账号、仓库名、`private` 可见性、分支和候选摘要，得到用户确认才可创建或更新本人账号下且默认不添加协作者的私密仓库。换电脑时另行生成只保存在本地、绝不进入 Git 的隐私迁移包，在目标设备通过固定投递目录安全导入。迁移包按稳定引用逐项允许，只带有恢复语义的正文与最小清单，不携带 `.gitkeep`、空占位、目录说明、缓存或凭据。即使目标是 GitHub 私密仓库，默认也不包含隐私正文或秘密凭据。
...
| `instance/evolution/` | 尚未完成正式晋升的学习候选 | 进化审核时 |
...
| GitHub 私密备份与本地隐私迁移 | 1.0 基线 | 已拆成三个动作；私密备份先做本地脱敏扫描，再确认账号、仓库、可见性、分支和候选摘要，才允许创建或推送；隐私包仍只在本地迁移 |
| 完整换机迁移套件 | 1.0 基线 | 固定五文件入口、主体包与隐私包分离、顶层及逐文件摘要、危险路径和篡改拒绝、0 项隐私包、实例身份、方向锁与交流方式恢复均已在 Windows 虚构实例中完成生成—恢复闭环；不把该结果写成其他系统已经实测 |
| 正式资产生命周期 | Asset Schema 1.2 基线 | 已实现保存前价值判断、直接授权、低风险可撤销试用、中高风险确认、失败复核与价值清理；待真实长期使用校准 |
...
| 看板整体架构 | 1.1.2 基线 | 已按“总览、随身资产、待办与成长、迁移与安全、当前状态”重构；学习建议投影来源、建议去向和下一步，复核／试用状态优先于旧成熟度，当前实例名称固定可见，正式界面与 Pages mock 共用同一套 UI；保留本地只读与完整自然语言动作边界，待真实用户长期反馈校准 |
...
用户明确授权可以直接记录；一次推断先成为候选；反复验证的低风险规律可以渐进成熟；身份、隐私、安全和高影响资产必须确认。
...
普通任务结束后先由模型、宿主和 Agent Carry 进行轻量判断。只有用户纠正、稳定偏好、可复用方法、重要失败修正、重复工作方式或长期状态变化等信号出现时，才生成候选并在必要时打扰用户。错误场景必须先修正并验证当前任务；错误或纠正本身不是自动沉淀授权。

### AC-014：同类学习通过元数据漏斗合并证据

新学习先生成小型检索特征，通过现有地图或可重建检索层返回少量候选，再读取候选正文判断重复、细化、条件分支、冲突、关联或新增。不得为去重全量加载记忆；无法确定时暂时分开。
...
### AC-024：GitHub 私密备份与本地隐私迁移是两条独立通道

GitHub 私密仓库只接收经过路径、类型、内容和候选集合检查的非隐私内容；原始隐私正文通过 `private_refs` 指向本地私密层。换机时另行生成不进入 Git 的本地隐私迁移包，秘密凭据仍在目标设备重新配置。本地洁净副本可以在动作开始后直接准备；创建或更新远程仓库前必须展示账号、仓库名、`private` 可见性、分支和候选摘要并等待用户确认。默认使用用户个人账号、不添加协作者，不得静默改为公开或共享仓库。

### AC-025：看板按用户目标组织，行星系统只承担品牌识别与导航

看板不再用内部资产分类、统计图和常驻检查面板主导首页，而是先回答用户今天做什么、能带走什么、怎样成长、怎样迁移和当前是否正常。行星、镂空核心、轨道与卫星是稳定视觉签名，卫星只提供深链接；产品 Logo 只表示 Agent Carry，当前实例名由固定身份区持续显示，避免一台电脑上的多个领域助手互相混淆。正式内容仍由小型快照投影，真实操作仍通过完整自然语言请求交给当前 Agent。前端可以使用成熟开源框架提高视觉与交互质量，但不能为了依赖数量迁移技术栈、劫持原生滚动或破坏离线、减少动画和普通用户可理解性。
