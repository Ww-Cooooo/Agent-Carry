...
- **当前阶段**：AI Carry `2.0.0` 正式版本；产品由 Agent Carry 改名，公开 1.4.8 与未公开 1.4.9 可直接通过既有看板升级链迁移。当前产品层和新生成身份使用 AI Carry，实例身份、历史、资产、Skill、工作区、本机与私密层保持，旧组件／快照／迁移包有界兼容；同一 Skill 新版接入和问题报告能力一并保留。实例替换仍须现场闭合固定标签、正式 Release、目标整树和本次用户确认；远程仓库改名是以后单独授权的动作
- **最近更新**：2026-08-31
...
| 实例持续进化与母版升级兼容 | 1.4.0 基线，韧性增强 | 所有 AI Carry 发起的正式持久变化命中同一轻量兼容入口，但不增加用户确认；原生资产与专业扩展复用现有所有者，独立组件使用小注册表。严格发布审计与操作型兼容读取分层：可确定表示漂移自动修复，旧格式迁移，单个可选组件故障局部隔离，必需或语义不明内容只暂停正式切换；每次错误、修复和隔离都自然语言说明，普通启动不读取完整协议或组件正文 |
...
| Skill 工坊与共享 Skill 接入 | 1.4.4 基线，1.4.9 升级闭环 | 推荐只读且不自动转换；用户选中的正式方法才复制为本地隔离草稿，由 Agent 自动脱敏、通用化和参数化，原资产不变。外来 Skill 只读检查结构、脚本、依赖、权限、冲突和隐私，不执行或静默改写；单包故障局部隔离。新生成 Skill 具有稳定身份与三段版本；用户主动导入同一 Skill 的更高版本时，展示新增／修改／删除路径并保留旧包前像后原子升级。旧包缺少身份、同版本漂移、降级或本机修改时只复核当前 Skill，不猜测覆盖。导出索引按需创建，不进入普通启动；分享、上传、安装和升级仍由用户明确决定 |
...
AI Carry 的正式新增、修改、删除、重命名、移动、迁移、升级和发布共用一份按需质量原则。优先级不能颠倒：先从根因修好问题，再证明用户数据与使用可靠性，最后减少无意义流程和用户负担。启动只保留“即将发生正式持久变更”的极小事件族；只读任务、普通使用和临时草稿不加载完整协议。模板、核心、Schema、安全、升级与发布由 Level 3 负责；实例正式资产仍按风险和生命周期分配模型等级。
