...
- **当前阶段**：`1.0.0` 首个公开基线；核心功能已经实现，已完成 Windows 隔离安装、完整迁移套件生成—恢复闭环，以及四组宿主／模型的实例化与两事件学习闭环验收，后续继续根据真实使用反馈改进
- **最近更新**：2026-08-16
...
安装负责取得完整项目、放入稳定本地目录、验证离线文件并创建一个可重建的桌面看板入口；接入负责让当前宿主理解 Agent Carry、恢复会话和按需使用资产；实例化负责让用户先选择交流方式，再选择通用或领域方向。三者是连续体验，但权限和完成条件不同：安装请求不自动授权 Git 操作、公开发布、秘密读取或替用户决定实例方向。
...
| `core/upgrade/` | 模板升级契约和发布清单 | 升级或发布时 |
...
| Agent 引导安装与桌面入口 | 1.0 基线 | 链接和压缩包共用详细安装真源；桌面入口指向完整离线看板，并由目标环境按系统语义重建；当前发布不把跨系统实测设为门禁 |
...
用户可以把 GitHub 中的完整安装说明链接交给 Agent，也可以把包含相同项目内容和 `START-HERE.txt` 的压缩包交给 Agent。两种方式最终都完整读取 `INSTALL.md`，安装到稳定本地目录，创建并验证一个指向根 `dashboard.html` 的桌面或等价明显入口。快捷方式是可重建适配，不是可携带真源；安装授权不外溢为 Git 建仓、推送、发布、秘密读取或静默覆盖现有实例。执行说明固定目标、冲突判断、安全边界和验收结果，由宿主根据当前系统选择等价实现。桌面位置必须来自当前系统的已知文件夹语义而不是主目录字符串拼接；完整验收同时要求入口位于用户实际可见位置、系统可回读正确目标与工作目录、并从入口真实观察到看板界面，宿主自己的成功声明不能替代这些证据。
