...
- **当前阶段**：AI Carry `2.0.2` 正式版本；首次创建、启动、学习、Skill 和升级均把有效核心结果与可重建投影分开，小故障只留下待修项，不拖死对话和其他能力。升级 `prepare` 现场核验官方来源一次，`confirm` 只复核同一预览绑定的本地事实；真实来源、目标、实例或写集漂移仍会拒绝。副本启动本机工具前只检查当前组件明确绑定，回指正式实例时只暂停这一项。公开 1.4.8、未公开 1.4.9、2.0.0 与 2.0.1 都可直接升级；用户内容继续保留。实例替换仍须闭合固定标签、正式 Release、目标整树和本次用户确认；远程仓库改名是以后单独授权的动作
- **最近更新**：2026-09-01
...
| 首次实例化身份与发布生命线 | 2.0.2 当前基线 | 正式创建由随包执行器接收一份已确认的紧凑语义请求，只生成并原子提交 manifest、正式档案和领域地图；模型不逐文件手写。空索引、注册表、胶囊和快照按能力需要延迟初始化或重建。代表性首次创建验证核心回滚、派生失败局部降级和二次幂等；已有实例升级另用复杂合成样例核对用户、私密、本机和扩展内容保留。普通用户不运行维护测试 |
