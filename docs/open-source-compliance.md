...
仓库当前没有随包照片、截图、插画、音视频、3D 模型、外部贴图、PDF 素材、WASM、闭源 SDK、付费字体或付费 UI 包。以后如果加入项目自己生成的文档截图，必须登记在 `docs/assets/project-assets.json`，保存页面来源、SHA-256 与项目许可证；第三方二进制素材还必须登记来源、版权、许可证和修改情况。自动门禁会拒绝未登记的二进制资产。
