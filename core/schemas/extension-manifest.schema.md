...
新写扩展清单使用 AI Carry 的可移植 TOML 子集，让不同宿主能由同一确定性加载器核对。旧扩展的其他合法 TOML 写法由模型外完整解析器在定向升级中规范化；格式差异只影响该扩展，不阻断实例主体。
...
- 发现 `workspace/**` 但没有有效清单时，不能递归接管、删除、迁移或猜测所有权；原样保留该工作区并列入定向冲突预览，其他能力继续。
...
如果在升级候选、故障复现或其他实例副本中执行扩展绑定的本机工具，复制目录本身不构成运行时隔离。执行前只把正式源根、候选根和该扩展本次明确使用的 `.assistant-local/**` JSON／TOML 绑定文件交给 `dashboard/scripts/copied-instance-runtime-boundary.mjs`；回指正式源实例、未核验／不存在的共享运行时、跨越链接／重解析点或未解析动态／相对路径只暂停当前扩展行为并标为 `limited`，不停止实例其他能力。预检不执行工具、不扫描无关目录，也不改写真实绑定。
...
4. 在副本执行本机工具前，明确绑定不回指正式源实例；失败只局部隔离当前扩展，源实例字节不变。
5. 多文件中断、损坏输入、深路径、未知文件和重复执行分别得到回滚、停止或零差异结果。
6. 升级后的看板快照只从合并后的实例真源重建，不把模板空态或扩展私密正文投影进去。
