...
`[entry]` 可以包含小型 `route_ids` 数组。每个 ID 必须在当前实例领域地图中存在，并指向扩展内一个真实入口或相关正式资产。清单不能替代正式记忆、能力、SOP 或经验的 frontmatter 和直接路线；专业工作区里的文档只有按资产生命周期获得授权、稳定 ID、正确 `kind` 和直接路线后，才能计入正式资产。
