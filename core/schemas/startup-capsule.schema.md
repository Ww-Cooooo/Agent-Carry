...
若上述一次派生修复成功，启动查询返回严格验证后的白名单投影和有界自然语言修复摘要，普通路由继续。若 manifest/core 无效、胶囊路径是链接或非普通文件、稳定读取失败，或一次派生修复仍未通过，查询只返回固定的 `startup-repair-required` 状态并说明影响、数据状态、可恢复性、仍可使用范围和下一步，然后停止普通路由；不得退回把原始清单、验证索引或全部资产加载进上下文来猜。
