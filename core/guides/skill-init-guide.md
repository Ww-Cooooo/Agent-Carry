...
4. 在 `instance/skills/requirements.toml` 写小型路由：稳定 ID、摘要、触发词、平台名称、实际入口、来源、状态、最后确认时间。Skill 自身只是宿主已有能力时不重复登记组件；如果它同时新增实例文件、本机运行时、模型或跨组件接口，再按实例持续进化兼容协定登记对应独立组件和设备本地边界。
