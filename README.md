...
> **当前版本：`1.4.6`。** 这次更新把 Skill 工坊的分享与接入做成了完整流程：生成前，Agent 会先问你要 ZIP、独立文件夹、分享链接还是只留在本机；随后只处理所选方法复制出的本地草稿，自动去掉实例专属和私密信息并检查内容。选择分享时会真正生成相应文件；选择只留在本机时只保留可编辑 Skill，不伪造分享文件。接入别人分享的 Skill 时，你可以给 Agent 文件夹、ZIP 或链接；Agent 会先在隔离位置检查，再只保留一次必要的安装确认。一个 Skill 出错只影响这一项，不会让对话、其他 Skill 或 Agent Carry 主体停下来。正式仓库、下载包和全新本地安装仍从未实例化的空模板开始。
...
当前正式版是 `1.4.6`。看板面向电脑使用，正式目标宽度为 1024px 及以上。每次发布都会从固定版本重新生成独立洁净候选，并检查每个模板文件应公开还是应排除；未登记的新文件会停止候选生成，不会被悄悄漏掉。模板空态、首次创建、隐私、秘密、许可证、离线运行和安装结果也会重新检查；这些维护工作不会变成普通用户日常使用时的负担。
