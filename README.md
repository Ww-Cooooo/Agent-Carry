...
> **当前版本：`2.0.4`。** 这一版修复了母版自检可能误阻断实例升级的问题：启动预算只从 `assistant.toml` 的结构化配置读取，不再依赖中文说明或 Schema 示例；升级后的实例只运行本次事务返回的接续检查，再继续原工作或完成一项相关的无破坏行为，不会把母版开发仓库的 `check:*` 全集、构建依赖或许可证工具当成实例门禁。2.0.3 的轻量兼容协定、局部容错、隐私和用户内容保护全部保留。公开 1.4.8、本地未公开的 1.4.9、2.0.0、2.0.1、2.0.2 和 2.0.3 都可直接升级。官方仓库仍是 `Ww-Cooooo/Agent-Carry`。
...
**[点击下载固定版本 AI Carry 2.0.4（ZIP，仅用于全新安装）](https://github.com/Ww-Cooooo/Agent-Carry/archive/refs/tags/v2.0.4.zip)**
...
上面的按钮固定到 `v2.0.4` 标签，下载后不会因为公开 `main` 继续前进而悄悄变成别的版本。GitHub 仓库的 Code → Download ZIP 仍是随正式 `main` 前进的“最新版”全新安装入口；每次正式发布都会核对公开 `main`、版本标签和 Latest Release 指向同一份已批准内容。两种 ZIP 都只用于全新安装，不是现有实例判断升级版本的依据；升级时，AI Carry 会先确认最新正式 Release 及其准确版本标签。解压后最外层文件夹可能仍带 `Agent-Carry` 仓库名，这是正常现象；安装 Agent 会在里面识别真正的项目根目录。安装 Agent 会先选择稳定目录，再检查项目是否完整，最后在桌面或系统中容易找到的位置创建“AI Carry 看板”入口。
...
当前正式版本是 `2.0.4`。看板面向电脑使用，正式目标宽度为 1024px 及以上。每次发布都会从固定版本重新生成独立洁净候选，并检查每个模板文件应公开还是应排除；未登记的新文件会停止候选生成，不会被悄悄漏掉。模板空态、首次创建、隐私、秘密、许可证、离线运行和安装结果也会重新检查；这些维护工作不会变成普通用户日常使用时的负担。
