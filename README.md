...
> **当前版本：`2.0.3`。** 实例以后安装或更新软件、Skill、组件，保存记忆、能力、SOP，或者跟随母版升级时，都会按同一份轻量兼容协定处理。协定管的是所有权、保留、隐私、回退和升级后能否继续使用，不要求每个实例使用一模一样的字段或目录。实例 Agent 会根据真实内容选择保留、适配、重连、重建或只暂停当前不兼容项；一个索引、工具或可选组件出错，不能拖停对话和其他能力。2.0.2 已建立的首次创建、派生状态修复、升级确认和复制运行时边界全部保留。身份、用户数据、私密内容和所有权确实无法判断时，仍只停止受影响的写入或正式切换。公开 1.4.8、本地未公开的 1.4.9、2.0.0、2.0.1 和 2.0.2 都可直接升级。官方仓库仍是 `Ww-Cooooo/Agent-Carry`。
...
**[点击下载固定版本 AI Carry 2.0.3（ZIP，仅用于全新安装）](https://github.com/Ww-Cooooo/Agent-Carry/archive/refs/tags/v2.0.3.zip)**
...
上面的按钮固定到 `v2.0.3` 标签，下载后不会因为公开 `main` 继续前进而悄悄变成别的版本。GitHub 仓库的 Code → Download ZIP 仍是随正式 `main` 前进的“最新版”全新安装入口；每次正式发布都会核对公开 `main`、版本标签和 Latest Release 指向同一份已批准内容。两种 ZIP 都只用于全新安装，不是现有实例判断升级版本的依据；升级时，AI Carry 会先确认最新正式 Release 及其准确版本标签。解压后最外层文件夹可能仍带 `Agent-Carry` 仓库名，这是正常现象；安装 Agent 会在里面识别真正的项目根目录。安装 Agent 会先选择稳定目录，再检查项目是否完整，最后在桌面或系统中容易找到的位置创建“AI Carry 看板”入口。
...
当前正式版本是 `2.0.3`。看板面向电脑使用，正式目标宽度为 1024px 及以上。每次发布都会从固定版本重新生成独立洁净候选，并检查每个模板文件应公开还是应排除；未登记的新文件会停止候选生成，不会被悄悄漏掉。模板空态、首次创建、隐私、秘密、许可证、离线运行和安装结果也会重新检查；这些维护工作不会变成普通用户日常使用时的负担。
