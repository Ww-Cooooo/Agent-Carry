...
> **当前版本：`1.4.5`。** 这次更新修复长时间使用同一对话时的升级连续性：文件升级后，Agent 会在当前原子操作结束的安全节点自动核对 Agent Carry 版本、接续最小新版规则并完成一项无破坏的代表行为验收；用户不必为了证明升级成功而专门新开任务或亲自执行测试。宿主确实不允许原地接续时，新任务才是最后的兼容路线。正式仓库、下载包和全新本地安装仍从未实例化的空模板开始。
