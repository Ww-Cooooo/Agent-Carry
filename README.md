...
> **当前版本：`1.4.7`。** 这次更新修复了复杂任务“事情已经做完，但最后忘了说这次真正用到了什么、学到了什么、文件放在哪里或下一步做什么”的问题。复杂任务、上下文压缩续接、实际使用长期内容、出现真实学习价值或交付文件时，Agent 会在发送前做一次本地收尾核对。缺什么只补当前回复，不联网、不建立后台任务库，也不会重跑已经完成的工作；检查器或本机 Node 暂时不可用时会明确说明并按同样事实继续收尾，业务结果、文件、对话和其他能力仍可使用。简单任务不会因此增加空回执或繁琐流程。正式仓库、下载包和全新本地安装仍从未实例化的空模板开始。
...
当前正式版是 `1.4.7`。看板面向电脑使用，正式目标宽度为 1024px 及以上。每次发布都会从固定版本重新生成独立洁净候选，并检查每个模板文件应公开还是应排除；未登记的新文件会停止候选生成，不会被悄悄漏掉。模板空态、首次创建、隐私、秘密、许可证、离线运行和安装结果也会重新检查；这些维护工作不会变成普通用户日常使用时的负担。
